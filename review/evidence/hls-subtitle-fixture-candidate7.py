...
if len(sys.argv) != 2:
    raise SystemExit("usage: hls-subtitle-fixture-candidate7.py <yt-dlp.exe>")
yt_dlp = pathlib.Path(sys.argv[1])
server = socketserver.ThreadingTCPServer(("127.0.0.1", PORT), Handler)
threading.Thread(target=server.serve_forever, daemon=True).start()
try:
    command = [str(yt_dlp), "--no-warnings", "-J", f"http://127.0.0.1:{PORT}/master.m3u8"]
    result = subprocess.run(command, capture_output=True, text=True, timeout=30)
    print("COMMAND=" + " ".join(command))
    print(f"EXIT_CODE={result.returncode}")
    print("SUBTITLE_ASSERTION=language=ko; nonempty_url=true; ext=vtt; protocol=m3u8_native")
    print("STDOUT_BEGIN")
    print(result.stdout)
    print("STDOUT_END")
    print("STDERR_BEGIN")
