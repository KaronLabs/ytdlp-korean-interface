...
- presenting KaronLabs-specific recovery, localization, validation, runtime-maintenance, or evidence work as independently authored work without attribution;
...
The system is not intended to make false claims technically impossible. It is intended to make those claims easy to disprove with primary-source evidence.
...
3. **Claim only KaronLabs work.** KaronLabs attribution applies to KaronLabs-specific modifications and recovery work, not unchanged upstream code.
4. **Use redundant evidence.** Human-readable documents, machine-readable metadata, Git history, release metadata, and checksums should all tell the same story.
5. **Prefer verifiable facts over threatening language.** The repository should look professional, not litigious.
6. **Separate copyright, provenance, and branding.** A right to use MIT-licensed code is not the same thing as a right to make false authorship claims or imply official endorsement.
7. **Keep maintenance cheap.** The provenance system must be simple enough that future versions can update it consistently.
...
KaronLabs-specific work includes, at minimum, the repository's documented Korean source recovery and associated hardening work, including:
...
The exact file-by-file boundary may evolve. The provenance documents therefore describe the KaronLabs work by repository history and canonical commits rather than pretending every line in the repository was authored by KaronLabs.

## Architecture

The Provenance Hardening Pack consists of five repository-level records plus a release discipline.
...
NOTICE
  └─ concise copyright and upstream/modification attribution
...
KaronLabs-specific attribution should be added through `NOTICE` and provenance metadata rather than by silently replacing ErrorFlynn's copyright notice.

### 2. `NOTICE`

`NOTICE` is the compact redistribution-facing attribution record.

It will contain:

- the direct upstream project and canonical URL;
- the baseline upstream version and commit;
- ErrorFlynn's existing copyright notice;
- a KaronLabs modification notice for KaronLabs-specific changes;
- a short summary of those modifications;
- a statement that the repository remains MIT-licensed;
- a statement that upstream and KaronLabs notices should be preserved with substantial redistributions where applicable under the MIT license.

The KaronLabs notice will avoid implying authorship of unchanged upstream code. Preferred wording:

`KaronLabs modifications and recovery work: Copyright (c) 2026 KaronLabs contributors.`

This keeps the claim scoped to the derivative contributions.
...
KaronLabs/ytdlp-korean-interface
  ↓ Korean recovery and hardening work
...
References to the existing design, test, and evidence locations, for example:
...
5. compare derivative source against the canonical repository when authorship is disputed.
...
- KaronLabs-specific derivative modifications;
...
It is guidance and factual provenance documentation, not an additional software-license restriction.
...
The document will identify examples that conflict with the recorded history, such as:
...
- “KaronLabs had no role in these Korean recovery changes,” when the implementation tracks the KaronLabs history;
- presenting a derivative as an official KaronLabs release or official KaronLabs course without authorization.

The language must avoid claiming that every misleading statement is automatically a license violation. It will instead say that such claims are inconsistent with the repository's documented provenance and may also raise separate copyright, attribution, passing-off, trademark, consumer-protection, or platform-policy issues depending on jurisdiction and facts.

### 5. `CITATION.cff`

The repository will add Citation File Format metadata so GitHub and compatible tools can expose a canonical citation.
...
- repository URL;
- MIT license;
- KaronLabs as the maintainer/derivative-work project identity;
- upstream relationship in the message/abstract field;
- current canonical version only once a KaronLabs release exists.
...
- KaronLabs-specific changes have a documented provenance trail;
- users should preserve required license notices;
- readers can use `NOTICE`, `PROVENANCE.md`, and `ATTRIBUTION.md` for the detailed lineage.

Preferred KaronLabs-style line:

> 코드는 가져가셔도 됩니다. 족보까지 새로 쓰지는 말아주세요.

The joke is optional flavor; the legal/factual sentence immediately around it must remain precise.
...
The implementation phase must not invent or silently generate a signing identity for the user. If no signing key or SSH signing configuration exists, repository documents and release discipline can still be implemented, while key setup is left as an explicit operator step.
...
Immutability is a release-integrity control, not a substitute for Git history or the attribution documents.

## Branding Boundary

The Provenance Hardening Pack distinguishes source-code rights from official-brand claims.

The repository will state that MIT permission to use the code does not itself imply:
...
## Dispute-Response Design

A lightweight `docs/provenance-dispute-response.md` may be added during implementation if it remains concise.

It should define a factual response sequence:
...
5. identify copied or derivative portions and upstream portions separately;
6. contact the publisher privately first when reasonable;
7. request correction of the authorship/origin claim rather than demanding removal of lawful MIT reuse;
...
The response guide must not automate harassment, mass reporting, doxxing, or retaliatory behavior.
...
LICENSE grants MIT rights
  ↓
NOTICE preserves concise attribution
  ↓
...
Do not delete or replace ErrorFlynn's notice. KaronLabs attribution is additive and scoped to modifications.

### Third-party contributions

If external contributors later submit substantial changes, provenance language should evolve from “KaronLabs modifications” to “KaronLabs and project contributors” where appropriate. Git history remains the detailed contribution record.

### Fork removes provenance files

The system cannot prevent deletion. The canonical repository and earlier immutable/signed releases remain evidence of the prior history.
...
- `CITATION.cff` exists;
- all four reference the canonical repository name `KaronLabs/ytdlp-korean-interface` where appropriate;
...
- `CITATION.cff` parses as valid YAML using only dependencies already available in the repository's test environment, or via a minimal deterministic parser if no YAML dependency exists.

If a new YAML dependency would be required only for this test, prefer a small structural text validation rather than adding a dependency solely for `CITATION.cff`.
...
- any statement that changes the MIT grant into a no-commercial-use or no-course-use restriction.
...
3. KaronLabs-specific derivative work is clearly identified without overclaiming upstream authorship;
4. `NOTICE`, `PROVENANCE.md`, `ATTRIBUTION.md`, and `CITATION.cff` exist and agree on repository lineage;
...
>
> Keep the required MIT notices, describe where it came from accurately, and do not turn a derivative work into a fake origin story.
